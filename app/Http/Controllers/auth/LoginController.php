    ...
    protected function authenticateWithFingerprint(Request $request)
{
    if ($request->hasFile('fingerprint_image')) {
        ...
        $fingerprintVerificationController = app()->make('App\Http\Controllers\Auth\FingerprintVerificationController');
        $response = $fingerprintVerificationController->verify($fingerprintRequest);

     
        if ($response->getData()->verified === true) {
            
            $user = User::find($response->getData()->user_id);
            Auth::login($user);
            return true;
        }
    }
...
//use below if depending on external api
    // protected function authenticateWithFingerprint(Request $request)
    // {
    //     if ($request->hasFile('fingerprint_image')) {
    //         $image = $request->file('fingerprint_image');
